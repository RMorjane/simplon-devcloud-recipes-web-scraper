...
class RecipeCategory:
    ...
    def read_infos(self):
        ...
        soup = BeautifulSoup(
            response.content, from_encoding=encoding, features="html.parser")
...
class RecipeInfo:
    ...
    def read_infos(self):
        ...
        self.persons_count = int(
            soup.find("option", {"class": "yield"}).string)
        self.list_usages = [loop_usage.string for loop_usage in soup.find_all(
            "li", {"class": "bold marginT10"})]
        ...
        if self.mysql_recipe.connection:
            ...
            if sql_recipe_id and type(sql_recipe_id) == tuple:
                ...
                self.recipe_id = self.mysql_recipe.add_recipe(
                    self.recipe_name, self.recipe_url, self.recipe_image)
                ...
                for k in list_li:
                    ...
                    if len(k_childs) == 2 and k_childs[0].name == "span" and k_childs[1].name == "span":
                        ...
                        usage_id = self.mysql_recipe.add_ingredient_usage(usage)

                        ingredient = " ".join(k_childs[0].text.strip()[
                                              0:final_index].split())
                        ingredient_id = self.mysql_recipe.add_ingredient(ingredient)
                        ...
                        self.mysql_recipe.add_recipe_ingredient(
                            self.recipe_id, ingredient_id, usage_id, self.persons_count, quantity, unit)
                ...
                steps_name = [step.string for step in soup.find_all(
                    "span", {"class": "bold"}) if ". POUR " in step.string]
                steps_description = soup.find_all(
                    "p", {"class": "marginT10 fz110 lh120"})
                ...
                for i in range(steps_count):
                    ...
                    self.mysql_recipe.add_step_recipe(
                        self.recipe_id, order, description)
    ...
    def display(self):
        print(self.recipe_id," : ",self.recipe_name)
...
class MySQLRecipe:
    ...
    def get_ingredient_id(self, ingredient_name):
        with self.connection.cursor() as my_cursor:
            ...
            my_cursor.execute(sql_ingredient_id,
    ...
    def add_recipe(self, recipe_name, recipe_url, recipe_image):
        with self.connection.cursor() as my_cursor:
            sql_add_recipe = "INSERT INTO recipe(recipe_name,recipe_url,recipe_image) VALUES('{0}','{1}','{2}')".format(
                recipe_name, recipe_url, recipe_image)
    ...
    def add_step_recipe(self, recipe_id, order, description):
        ...
        with self.connection.cursor() as my_cursor:
            ...
            my_cursor.execute(
                sql_add_step, (order.replace("'", "\'"),
                               recipe_id, description.replace("'", "\'"))
            )
            self.connection.commit()
            ...
            with self.connection.cursor() as my_cursor:
                ...
                for recipe in my_cursor.fetchall():
                    self.list_recipes.append({
                        ...
                        "recipe_url": recipe[2],
                        "recipe_image": recipe[3],
                    })
                my_cursor.close()
                self.logger.info("Successfull : Recipes was founded")
        except (Exception, psycopg2.Error) as error:
            self.logger.error(
                "Erreur dans la requête de selection des recettes : "+str(error))
