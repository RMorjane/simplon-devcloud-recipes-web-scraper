...
class MySQLRecipe:
    ...
    def create_database(self):
        try:
            with self.connection.cursor() as my_cursor:
                ...
                """
                my_cursor.execute(sql_create_tables,multi=True)
                my_cursor.close()
            return True
        except (Exception, mysql.connector.Error) as error:
            print("Impossible de créer les tables au niveau du serveur mysql : " + str(error))
    ...
    def add_ingredient(self,ingredient_name):
        ...
        if sql_ingredient_id and type(sql_ingredient_id)==tuple:
            ...
            with self.connection.cursor() as my_cursor:
                ...
                my_cursor.execute(sql_add_ingredient %(ingredient_name.replace("'","\\'")))
                self.connection.commit()
                ingredient_id = my_cursor.lastrowid
                my_cursor.close()
    ...
    def add_ingredient_usage(self,usage_name):
        ...
        if sql_usage_id and type(sql_usage_id)==tuple:
            ...
            with self.connection.cursor() as my_cursor:
                ...
                my_cursor.execute(sql_add_usage %(usage_name.replace("'","\\'")))
                self.connection.commit()
                usage_id = my_cursor.lastrowid
                my_cursor.close()
        return usage_id
    ...
    def add_recipe_ingredient(self,recipe_id,ingredient_id,usage_id,persons_count,quantity,unit):
        ...
        with self.connection.cursor() as my_cursor:
            ...
            """
            my_cursor.execute(
                sql_add_recipe_ingredient %(recipe_id,ingredient_id,usage_id,persons_count,quantity,str(unit))
            )
            self.connection.commit()
